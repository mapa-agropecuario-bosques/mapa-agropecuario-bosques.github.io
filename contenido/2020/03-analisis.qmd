...
2. Visualice la capa raster y la capa vectorial en QGIS.

3. Agregue un bloque de código R al documento con el siguiente contenido:
...
# Lectura del vector (archivo geopackage con polígono)
cartago_sf <- st_read("cartago.gpkg")

# Convertir el objeto sf a 'SpatVector' de terra
cartago_vect <- vect(cartago_sf)

# Recortar el raster con base en el polígono
raster_crop <- crop(raster_agro_bosques, cartago_vect)

# Aplicar la máscara para conservar solo los valores dentro del polígono
raster_cartago <- mask(raster_crop, cartago_vect)

# (Opcional) Guardar el resultado en un archivo raster
# writeRaster(raster_cartago, "salidas/cartago_clip.tif", overwrite = TRUE)
```
